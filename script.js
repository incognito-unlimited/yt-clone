const getTrendingVideos = async () => {
    try {
        const resp = await fetch("https://youtube138.p.rapidapi.com/v2/trending", {
            method: "GET",
            headers: {
                "x-rapidapi-host": "youtube138.p.rapidapi.com",
                "x-rapidapi-key": "ea47e0975bmsh5c16b3fa72ee233p1f781ajsnb00503932d41",
            },
        });

        if (!resp.ok) {
            console.error('Trending API error', resp.status, resp.statusText);
            let errText = '';
            try { errText = await resp.text(); } catch (e) { /* ignore */ }
            console.error('Response body:', errText);
            const errMsg = document.createElement('p');
            errMsg.textContent = 'Could not load trending videos (rate limit or server error).';
            rootElem.appendChild(errMsg);
            return;
        }
        ...
        console.error('Network or parsing error while fetching trending:', err);
        const errMsg = document.createElement('p');
        errMsg.textContent = 'Network error while loading trending videos.';
        rootElem.appendChild(errMsg);
    }
};
...
const showTrendingVideos = (data) => {
    ...
    if (!Array.isArray(list)) {
        console.error('showTrendingVideos: expected data.list array, got:', data);
        return;
    }
